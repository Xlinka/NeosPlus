...
public class ValueInequalityDriver<T> : Component
{
	...
	protected override void OnChanges()
	{
		if (Target.IsLinkValid)
		{
			...
			bool value = (!UseApproximateComparison.Value || !Coder<T>.SupportsApproximateComparison) ? EqualityComparer<T>.Default.Equals(val, Reference.Value) : Coder<T>.Approximately(val, Reference.Value);
