...
namespace FrooxEngine
{
    ...
    public class Cloth : MeshRenderer // Need to extend MeshRenderer to avoid Unity attaching its own
    {
        ...
        public override void BuildInspectorUI(UIBuilder ui)
        {
            ...
            ui.Button("Add all colliders under the canidate slots".AsLocaleKey())
                .SetUpActionTrigger(SetupSlotColliders);
        ...
        [ImpulseTarget]
        public void ClearCoefficients() => PinningCoefficients.Clear();

        [ImpulseTarget]
        public void ClearColliders() => ClothColliders.Clear();

        [ImpulseTarget]
        public void Reset() => ShouldReset.Value = true;
    }
}